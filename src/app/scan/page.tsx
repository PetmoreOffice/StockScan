...
export default function ScanPage() {
  ...
  const savedBy = `${nickname.trim()}-${fullName.trim()}`;
  const recorderNamed = Boolean(nickname.trim() && fullName.trim());
  ...
  useEffect(() => {
    ...
    setReady(true);
    setNickname(window.localStorage.getItem("scan-nickname") ?? "");
    setFullName(window.localStorage.getItem("scan-full-name") ?? "");
    void loadBranches();
  }, [router]);
  ...
  async function saveScan(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!selectedBranch || !product || !recorderNamed) { setNotice({ type: "error", text: "กรุณาเลือกสาขา สแกนสินค้า และกรอกทั้งชื่อเล่นและชื่อจริงของผู้บันทึก" }); return; }
    savingRef.current = true;
    setSaving(true);
    setNotice(null);
    try {
      window.localStorage.setItem("scan-nickname", nickname.trim());
      window.localStorage.setItem("scan-full-name", fullName.trim());
      const token = await getInventoryToken();
      const response = await fetch("/api/scans", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...product, branchKey, branchName: selectedBranch.name, quantity, expiryDate, savedBy }) });
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-background px-3 py-3 sm:px-6 sm:py-6"><div className="mx-auto max-w-3xl space-y-4">
      ...
      <Card className="overflow-hidden"><CardHeader className="border-b border-border/70 bg-white/[.025]"><CardTitle className="flex items-center gap-2"><MapPin className="h-5 w-5 text-primary" />ตั้งค่ารายการรับเข้า</CardTitle><p className="text-sm text-muted-foreground">เลือกข้อมูลนี้หนึ่งครั้ง แล้วสแกนต่อเนื่องได้เลย</p></CardHeader><CardContent className="grid gap-4 pt-5 sm:grid-cols-2"><div className="space-y-2 sm:col-span-2"><Label htmlFor="branch">สาขาปลายทาง</Label><Select id="branch" value={branchKey} disabled={branchesLoading} onChange={(event) => setBranchKey(event.target.value)}><option value="">{branchesLoading ? "กำลังโหลดสาขา…" : "เลือกสาขา"}</option>{branches.map((branch) => <option key={branch.key} value={branch.key}>{branch.name}</option>)}</Select></div><div className="space-y-2"><Label htmlFor="nickname">ชื่อเล่นผู้บันทึก</Label><div className="relative"><CircleUserRound className="pointer-events-none absolute left-4 top-4 h-5 w-5 text-primary" /><Input id="nickname" value={nickname} onChange={(event) => setNickname(event.target.value)} placeholder="อาย" className="pl-12" /></div></div><div className="space-y-2"><Label htmlFor="fullName">ชื่อจริงผู้บันทึก</Label><Input id="fullName" value={fullName} onChange={(event) => setFullName(event.target.value)} placeholder="กฤติยาภรณ์" /></div><p className="text-xs leading-5 text-muted-foreground sm:col-span-2">บันทึกลงรายงานเป็น <strong className={recorderNamed ? "text-primary" : ""}>{recorderNamed ? savedBy : "ชื่อเล่น-ชื่อจริง"}</strong></p></CardContent></Card>
